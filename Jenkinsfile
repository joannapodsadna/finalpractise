 ...
 stage ("Deploy Application on Azure Kubernetes Service")
 {
  switch (env.BRANCH_NAME) {
    ...
    default:
        ...
        sh("sudo kubectl --kubeconfig ~jenkinsdemo5/.kube/config get ns ${appName}-${env.BRANCH_NAME} || sudo kubectl --kubeconfig ~joanna/.kube/config create ns ${appName}-${env.BRANCH_NAME}")
        // Don't use public load balancing for development branches
        sh("sed -i.bak 's#${appRepo}#${imageTag}#' ./k8s/dev/*.yaml")
        sh("sudo kubectl --kubeconfig ~jenkinsdemo5/.kube/config --namespace=${appName}-${env.BRANCH_NAME} apply -f k8s/dev/")
